...
router.get('/', function(req, res, next) {
    ...
    console.log("## get request_login");
    res.render('result_page_login', { title: 'Express', id: id, email: email, method: "get" });
});
...
router.route('/').post(function(req, res) {
    ...
    var pw = req.body.pw;
    console.log(id);
    /*var sql = `SELECT * FROM USER WHERE USER_REAL_ID = ${id}`;
    db.query(sql, function (err, result, fields) {
      if (err) throw err;
      if(result[0].USER_REAL_ID != id || result[0].USER_PASSWORD != pw){
        res.redirect('/login');
      }
      else{
        if (req.session.user) {
  ...
  /*            res.render('index', {
    ...
    console.log("## post request_login");
