...
///////////////////////////////////////////////////////////////////////////////////////////////
      ...
      if (hostOrClient == 'client') {
         ...
         socket.on('command-from-host-to-all-clients', function( msg) {
            ...
            if (type == 'resize') {
               ...
               if (command == 'normal') {
                  videoMirror.width = screen.width * 7 /10, videoMirror.height = screen.height * 7 /10;
               } else {
   ...
   function openDataChannel( isInitiator) {
      ...
      cl.rtc.pc.onicecandidate = function (evt) {
         if (evt.candidate) {
           ...
           console.log('from ' + cl.rtc.user1 + 'to ' + cl.rtc.user2);
      ...
      // Client-side data channel
         ...
         cl.rtc.pc.ontrack = function (evt) {
           console.log("client ontrack123 : " + evt.streams[0] + evt);
            videoMirror.srcObject = evt.streams[0];
         };
         /*
         cl.rtc.pc.addEventListener('track', function (evt) {
           console.log("client ontrack123 : " + evt.streams[0] + evt);
            videoMirror.srcObject = evt.streams[0];
         });*/
      }
  ...
  function call(){
    ...
    try {
          ...
          videoStream.getTracks().forEach(track => cl.rtc.pc.addTrack(track, videoStream));
   ...
   function startVideoStream() {
      if (!videoStream) {
        startCapture(clientdisplayMediaOptions).then(function(stream1){
           videoStream = stream1;
           call();
        });
        ...
        hostvideo.addEventListener('play', function() {
          ...
          (function loop() {
            if (!$this.paused && !$this.ended) {
              ...
              ctxtest.drawImage($this, 0, 0);
              setTimeout(loop, 30); // drawing at 60fps
            }
          })();
        }, 0);*/
        ...
        hostvideo.oncanplay = maybeCreateStream;
        if (hostvideo.readyState >= 3) { // HAVE_FUTURE_DATA
        ...
        hostvideo.play();
         videoStream = hostvideo.captureStream(); //60
         call();*/
   ...
   function createOffer() {
      cl.rtc.pc.createOffer(offerOptions)
      .then( function( offer) {
        console.log('createOffer() : setLocalDescription success');
   ...
   function handleOffer( msg) {
      ...
      console.log('handleOffer() : setRemoteDescription success');
      cl.rtc.pc.setRemoteDescription( msg)
      .then(function() {
         return cl.rtc.pc.createAnswer( );
      })
      .then(function( answer) {
        console.log('handleOffer() : setLocalDescription success');
   ...
   function handleAnswer( answer) {
     console.log('handleAnswer() : setRemoteDescription success');
   ...
   */
   ...
   function handleMouseOrTouchMove( e, fromListener) {
      /*if (twoThumbs.enabled) {
         ...
         var allData = { "mouseX" :  mK.mX, "mouseY" : mK.mY, "eventType" : Type, "key" : keyInput , "LorR" : LorR};
         $.ajax({
             url: "/mouse_move",
             type: "POST",
             dataType: "json",
